    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 60px)', fontFamily: 'system-ui', backgroundColor: '#fdfdfd' }}>
      ...
      <aside style={{ width: '480px', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', backgroundColor: '#f8fafc', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <textarea value={prefix} onChange={e => setPrefix(e.target.value)} style={{ width: '100%', minHeight: '60px', padding: '0.5rem', fontSize: '0.8rem', fontFamily: 'monospace', borderRadius: '4px', border: '1px solid #e2e8f0', backgroundColor: '#fff' }} />
          </div>
                  ...
                  return (
                    <div key={idx} style={{ padding: '0.75rem', borderRadius: '6px', border: '1px solid #e2e8f0', backgroundColor: ovr.excluded ? '#f1f5f9' : '#fff', opacity: ovr.excluded ? 0.6 : 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                        <span style={{ fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8' }}>{msg.role.toUpperCase()} (T-{idx})</span>
                        <div style={{ display: 'flex', gap: '0.4rem' }}>
                          <button onClick={() => setOverrides(prev => ({ ...prev, [idx]: { ...prev[idx], excluded: !ovr.excluded } }))} style={{ fontSize: '0.6rem', cursor: 'pointer', border: '1px solid #e2e8f0', background: '#fff', borderRadius: '3px' }}>{ovr.excluded ? 'Include' : 'Exclude'}</button>
                        </div>
                      </div>
                      <textarea 
      ...
      <section style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: '#fff' }}>
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: '0.7rem', color: '#94a3b8' }}>{activeProfile.name} • {activeProfile.model}</div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '2rem' }}>
          <div style={{ maxWidth: '700px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
            {transcript.map((msg, i) => {
              ...
              const isEdited = ovr?.content !== undefined;
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ 
                        ...
                        lineHeight: 1.6, 
                        fontSize: '0.95rem', 
                        color: isExcluded || isEdited ? '#94a3b8' : '#334155',
                        textDecoration: isExcluded || isEdited ? 'line-through' : 'none',
                        flex: 1 
                      }}>
                      ...
                      {isUser && msg.contextSnapshot && (
                        <button 
                          ...
                          style={{ marginLeft: '1rem', fontSize: '0.65rem', color: '#3b82f6', background: 'none', border: '1px solid #bfdbfe', borderRadius: '4px', padding: '0.2rem 0.4rem', cursor: 'pointer', whiteSpace: 'nowrap' }}
                        >
